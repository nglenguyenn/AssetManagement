...
namespace Rookie.AssetManagement.Controllers
{
	...
	public class AccountController : ControllerBase
	{
		...
		{
			var identity = HttpContext.User.Identity as ClaimsIdentity;
			return await _accountService.ChangePasswordFirstTime(identity, accountChangePasswordFirstTimeDto);
		}
		[Authorize]
		[HttpGet]
		...
		{
			var identity = HttpContext.User.Identity as ClaimsIdentity;
			return await _accountService.CheckIfPasswordChanged(identity);
        }
        ...
        public async Task<IActionResult> ChangePassword([FromBody] AccountChangePasswordDto accountChangePasswordDto)
        {
            ...
            {
				var identity = HttpContext.User.Identity as ClaimsIdentity;
				return await _accountService.ChangePassword(identity, accountChangePasswordDto);
			}
			return BadRequest();
        }
		[Authorize]
